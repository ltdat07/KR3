...
# --- ПЕРЕД загрузкой gateway‑app подменяем адреса ---
# чтобы внутренний httpx.AsyncClient ходил на контейнеры, которые проброшены на хост
os.environ["ORDERS_URL"] = "http://127.0.0.1:8001"
os.environ["PAYMENTS_URL"] = "http://127.0.0.1:8002"

# Динамически загружаем API‑Gateway из /app/app.py
...
def test_e2e_order_payment_flow(user_id):
    ...
    # 3) даём сервисам время на обмен сообщениями через Kafka
    time.sleep(2)

    # 4) проверяем, что статус заказа FINISHED
